...
Public Function ValidActivityDate(month As String, day As String, year As String) As Boolean
    ...
    'something like 30/11/2021 will pass this test when the expectation is that it shouldn't depending on the locale.
    'Outside of a custom-built or third party control, this behavior is baked-in and isn't going to be prevented.
    If Not IsDate(BuildDateValue(month, day, year)) Then
        Call ShowDateError
        ValidActivityDate = False
        Exit Function
    End If
...
Public Function BuildDateValue(month As String, day As String, year As String) As String
    BuildDateValue = month & "/" & day & "/" + year
End Function
